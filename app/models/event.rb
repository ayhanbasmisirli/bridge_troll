...
class Event < ActiveRecord::Base
  ...
  has_many :attendee_rsvps,  -> { where(role_id: [Role::STUDENT.id, Role::VOLUNTEER.id], waitlist_position: nil) }, class_name: 'Rsvp'

  has_many :student_rsvps, -> { where(role_id: Role::STUDENT.id, waitlist_position: nil) }, class_name: 'Rsvp'
  has_many :student_waitlist_rsvps, -> { where("role_id = #{Role::STUDENT.id} AND waitlist_position IS NOT NULL") }, class_name: 'Rsvp'
  ...
  has_many :volunteer_rsvps, -> { where(role_id: Role::VOLUNTEER.id) }, class_name: 'Rsvp'
  ...
  has_many :organizer_rsvps, -> { where(role_id: Role::ORGANIZER.id) }, class_name: 'Rsvp'
  ...
  has_many :event_sessions, -> { order('ends_at ASC') }, dependent: :destroy
